package ballzeroth.main;

/**
 *
 * @author martin.akretzschmar
 */
public class SpriteIDs {

    //Building the map
    public static final String imagesDIR = ".\\src\\ballzeroth\\resources\\Scenery_Ballzeroth.png";
    public static final String mapsDIR = ".\\src\\ballzeroth\\resources\\mapOne.txt";

    //Building the HUD
    public static final String buttonDIR = ".\\src\\ballzeroth\\resources\\button_dir.png";
    public static final String hearthDIR = ".\\src\\ballzeroth\\resources\\Heart.png";
    public static final String coinDIR = ".\\src\\ballzeroth\\resources\\Coin.png";

    //Building Enemies
    public static final String enemyDIR = ".\\src\\ballzeroth\\resources\\Orc_Bot.png";

    //sounds and songs
    public static final String mainMusic = ".\\src\\ballzeroth\\resources\\battlesong.wav";
    public static final String nature = ".\\src\\ballzeroth\\resources\\naturesound.wav";
    public static final String wave = ".\\src\\ballzeroth\\resources\\tamborsound.wav";
    public static final String zugzug = ".\\src\\ballzeroth\\resources\\zugzug.wav";
    
    public static final int roadID = 0;
    public static final int buildID = 1;
    public static final int terrainID = 2;
    public static final int humanTowerOneID = 6;
    public static final int humanTowerOneIDMOUSE = 7;
    public static final int humanTowerOneIDHUD = 8;
    public static final int humanTowerTwoID = 9;
    public static final int humanTowerTwoIDMOUSE = 10;
    public static final int humanTowerTwoIDHUD = 11;
    public static final int humanTowerThreeID = 12;
    public static final int humanTowerThreeIDMOUSE = 13;
    public static final int humanTowerThreeIDHUD = 14;
    public static final int orcTowerOneID = 15;
    public static final int orcTowerOneIDMOUSE = 16;
    public static final int orcTowerOneIDHUD = 17;
    public static final int orcTowerTwoID = 18;
    public static final int orcTowerTwoIDMOUSE = 19;
    public static final int orcTowerTwoIDHUD = 20;
    public static final int orcTowerThreeID = 21;
    public static final int orcTowerThreeIDMOUSE = 22;
    public static final int orcTowerThreeIDHUD = 23;
    
    public static final int endTowerUp = 24;
    public static final int endTowerMid = 25;
    public static final int endTowerDown = 26;

    public static final int trash = 28;
    public static final int trashBig = 29;

    public static final int enemyOrcID = 0;
    public static final int enemyHumanID = 1;
    
    
}
